...
namespace TestUngDung.Areas.Admin.Controllers
{
	public class ProductController : BaseController
	{
		...
		public ActionResult Index(string search, int? page)
		{
			...
			IEnumerable<ProductViewModel> result = (from p in db.Product
		...
		public ActionResult Create(Product model, HttpPostedFileBase imageSave)
		{
			try
			{
				...
				var db = new NguyenThanhDatDBEntities();
				...
				model.Image = "/Assets/Admin/images/" + _filename;
		...
		public ActionResult Edit(Product model, HttpPostedFileBase imageSave)
		{
			try
			{
				...
				string oldImgPath = "";
				var db = new NguyenThanhDatDBEntities();
				var product = db.Product.SingleOrDefault(x => x.ID == model.ID);
				SetViewBag();
				if (imageSave == null)
				{
				...
				else
				{
					...
					string _filename = DateTime.Now.ToString("ddMMyyyy") + filename;
					string extension = Path.GetExtension(imageSave.FileName);
					path = Path.Combine(Server.MapPath("~/Assets/Admin/images"), _filename);
					model.Image = "/Assets/Admin/images/" + _filename;
